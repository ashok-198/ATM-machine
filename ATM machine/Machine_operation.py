...
def withdraw (id, amount, client_data, machine_data, condition_check, pin_check, multiple_of_100, balance_check, min_limit_check, max_limit_check, machine_balance_check):
    ...
    while flag:
        ...
        if condition_check  :
            ...
            transactions_entry(id, amount, call_from= 'client_withdraw')
            flag = False 
        ...
        try:
            ...
            if deposit_amount == 00:
                ...
                if get == 99:
                    ...
                    transactions_entry(id, deposit_amount, call_from= 'client_deposit')
...
#____________________________________________________________________________       Transactions Book
def transactions_entry(id, amount, reciver= None, call_from = 'client_deposit'):  # if call_from = 'client_deposit' and call_from = 'client_withdraw' --> reciver= None
    with open ('transactions_book.json', 'r') as file:                            # elif call_from = 'transfer' --> reciver= reciver_id
    ...
    if call_from in ['client_withdraw', 'client_deposit']:
        ...
        if len(update_statement) >= 5:
            update_statement.pop()

        if call_from == 'client_withdraw':

            action = 'Withdraw'
            update = f'{current_DateTime}______{action}___₹ -{amount}'
            update_statement.insert (0, update)
        ...
        mini_statements[id] = update_statement  
    
    
    elif call_from == 'transfer':
        update_statement_debit, update_statement_credit = mini_statements[id], mini_statements[reciver]

        if len(update_statement_debit) >= 5:
            update_statement_debit.pop()
        if len(update_statement_credit) >= 5:
            update_statement_credit.pop()

        action_1, action_2 = 'Debit','Credit'
        update_debit = f'{current_DateTime}_______{action_1}.____₹ -{amount}'
        update_statement_debit.insert (0, update_debit)

        update_credit = f'{current_DateTime}_______{action_2}____₹ +{amount}'
        update_statement_credit.insert (0, update_credit)

        mini_statements[id] = update_statement_debit
        mini_statements[reciver] = update_statement_credit
...
#____________________________________________________________________________       Transfer Option
def transfer(id, client_data):


    sender = client_data[id]

    while True:
        
        if sender['wrong_PIN_limit'] <= 0:
            print("\nWrong PIN limit reached, can't proceed. Please try after 24 hours \n(or) \nVisit Branch office.")
            sleep(3)
            break
        try:
            reciver_id = input(f'\n\t\t   Please enter reciver ID \n\ninput "0" to exit\n\n    ==> ')
            ...
            elif reciver_id not in client_data:
                print(f'\n  **  ID "{reciver_id}" given is not available. Please enter a valid ID\n')
                sleep(3)
                continue
            ...
            while True:
                try:
                    ...
                    elif amount > sender['client_available_balance']:
                        print(f'\n  **  Amount not available in your account. Please try lower amount  **')
                        continue

                    elif amount > sender['max_limit'] or amount < 1000:
                        print(f'\n  **  Maximum allowed is ₹ 50,000 and Minimum allowed is ₹ 1,000  \n Try again  **')
                        sleep(2)
                        continue

                    c_PIN = int(input(f'\nPlease enter your PIN \n  ==> '))

                    if c_PIN != sender['client_PIN']:
                        print(f'\n  **  Wrong PIN, try again')
                        sleep(3)
                        sender['wrong_PIN_limit'] -= 1
                        ...
                        break
                    r_name = client_data[reciver_id]['client_name']
                    print(f'\n**  Sending ₹ {amount} to ID-> "{reciver_id}" Name-> "{r_name}"  **')
                    get = input(f'\nPress "99" to confirm \nor "00" to cancel \n\n   ==> ')
                    ...
                    if get == "99":
                        ...
                        with open ('client_data.json', 'r') as file:
                            client_data = load(file)
                            client_data[id]['client_available_balance'] -= amount
                            client_data[reciver_id]['client_available_balance'] += amount

                        with open ('client_data.json', 'w') as file:
                            dump(client_data, file, indent=4)

                        transactions_entry(id, amount, reciver=reciver_id, call_from= 'transfer')
